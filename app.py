...
# --- GESTIÓN DE MEMORIA PARA EL RADAR ---
    ...
    for sector, tickers in universo.items():
        for t in tickers:
            try:
                ...
                datos.append({
                    "Ticker": t, "Estado V10": estado, "Precio": round(p, 2), 
                    "Margen %": round(m, 1), "Sector": sector
                })
            except: continue
            contador += 1
...
# --- TAB 1: RADAR (CON BOTÓN MANUAL) ---
with tab1:
    st.subheader("Radar de Oportunidades")
    
    # El botón que controla el escaneo
    if st.button("🚀 INICIAR ESCANEO DE MERCADO"):
        with st.spinner('Auditando universo V10...'):
            st.session_state.df_radar = cargar_radar()
            st.rerun()

    # Si ya hay datos en memoria, los mostramos
    if not st.session_state.df_radar.empty:
        ...
        st.info("⚠️ Radar en reposo. Presiona el botón para escanear.")
...
# --- TAB 3: SENTIMIENTO (BAJO DEMANDA) ---
with tab3:
